...
/**
* @author Lu Chang cai 
* @date : 2018年4月19日 下午9:24:222018
*/
@SuppressWarnings("unused")
public class newsOfChina {
	...
	public newsOfChina(int newsID,String headline,Timestamp releaseTime,String mainBody,String pictureOne,String pictureTwo,String pictureThere,int numberOfLike,String sourceName,
String digestOfNews){
		this.newsID = newsID;
		...
		this.pictyreThere=pictureThere;
	...
	public newsOfChina(int counter) throws SQLException {
		super();
		Connection conn = JdbcOfNews.getConnection();
		String sql = "" + "SELECT * FROM newsofChina WHERE NewsID=?";
		...
		while (rs.next()) {
			...
			this.pictyreThere = rs.getString("pictureThere");
			this.numberOfLike = rs.getInt("numberOfLike");
			this.sourceName = rs.getString("sourceName");
			this.numberOfTranspond = rs.getInt("numberOfTranspond");
		...
		String str2 = "" + "CREATE TABLE IF NOT EXISTS ChinaComment" + this.newsID + "("
					+"commentID SMALLINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,"
					+"UserID VARCHAR(50),"
					+"Content MEDIUMTEXT,"
					+"CommentTime DATETIME)ENGINE=InnoDB DEFAULT CHARSET=utf8;";
		
		PreparedStatement ps = (PreparedStatement) conn.prepareStatement(str2);
		if(!ps.execute()) {
			
			System.out.println("NewsOfChina"+this.newsID+"的评论表："+"ChinaComment"+this.newsID+"已创建成功");
		}

//		// 每次调用构造方法后 该static变量都会自增一 以便后续调用时候出现重复对象
//		StructureNumber++;
		// 资源释放
		conn.close();
	}
	...
	public List<comment> ReturnComments() {
		...
		Connection conn = JdbcOfNews.getConnection();
		String sql = "" + "SELECT * FROM Chinacomment"+this.newsID;
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
				comments.add(new comment(rs.getInt("commentID"),rs.getString("userID"),rs.getString("content"),rs.getTimestamp("commentTime")));
			}
	...
	public void addLike() throws SQLException {
		...
		String str = "" + "UPDATE newsOfChina SET numberOfLike=numberOfLike+1 WHERE NewsID=?";
	...
	public void comment(String comment,String UserID) {   //这里需要前端页面传来的操作账户的ID账号  并将其储存进MySQL comment表格中
		...
		String str1 = "" + "INSERT Chiancomment"+this.newsID+"(UserID,Content,CommentTime) VALUES('"+UserID+"','"+comment+"','"+time+"');";
