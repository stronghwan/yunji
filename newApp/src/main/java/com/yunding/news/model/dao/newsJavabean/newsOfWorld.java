...
public class newsOfWorld {
	...
	public newsOfWorld(int newsID,String headline,Timestamp releaseTime,String mainBody,String pictureOne,String pictureTwo,String pictureThere,int numberOfLike,String sourceName,
	String digestOfNews){
		this.newsID = newsID;
	...
	public newsOfWorld(int counter) throws SQLException {
		super();
		Connection conn = JdbcOfNews.getConnection();
		String sql = "" + "SELECT * FROM newsofWorld WHERE NewsID=?";
		...
		String str2 = "" + "CREATE TABLE IF NOT EXISTS WorldComment" + this.newsID + "("
		...
		if(!ps.execute()) {
			...
			System.out.println("NewsOfWorld"+this.newsID+"的评论表："+"WorldComment"+this.newsID+"已创建成功");
		}
...
//		StructureNumber++;
	...
	public List<comment> ReturnComments() {
		...
		Connection conn = JdbcOfNews.getConnection();
		String sql = "" + "SELECT * FROM Worldcomment"+this.newsID;
		try {
