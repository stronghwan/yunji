...
public class newsOfYunding {
	...
	public newsOfYunding(int newsID,String headline,Timestamp releaseTime,String mainBody,String pictureOne,String pictureTwo,String pictureThere,int numberOfLike,String sourceName,
			String digestOfNews){
		this.newsID = newsID;
	...
	public newsOfYunding(int counter) throws SQLException {
		super();
		Connection conn = JdbcOfNews.getConnection();
		String sql = "" + "SELECT * FROM newsofYunding WHERE NewsID=?";
		...
		String str2 = "" + "CREATE TABLE IF NOT EXISTS YundingComment" + this.newsID + "("
		...
		if(!ps.execute()) {
			...
			System.out.println("NewsOfYunding"+this.newsID+"的评论表："+"YundingComment"+this.newsID+"已创建成功");
		}
	...
	public List<comment> ReturnComments() {
		...
		Connection conn = JdbcOfNews.getConnection();
		String sql = "" + "SELECT * FROM Yundingcomment"+this.newsID;
		try {
	...
	public void addLike() throws SQLException {
		...
		String str = "" + "UPDATE newsOfYunding SET numberOfLike=numberOfLike+1 WHERE NewsID=?";
	...
	public void comment(String comment,String UserID) {   //这里需要前端页面传来的操作账户的ID账号  并将其储存进MySQL comment表格中
		...
		String str1 = "" + "INSERT Yundingcomment"+this.newsID+"(UserID,Content,CommentTime) VALUES('"+UserID+"','"+comment+"','"+time+"');";
