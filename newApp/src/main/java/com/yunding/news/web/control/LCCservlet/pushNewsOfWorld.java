...
*/
public class pushNewsOfWorld extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String str = "SELECT hasBrowse FROM browseWorld WHERE userID=" + userID;
		...
		try {
			...
			if (rs == null) {
				//如果不存在阅读记录 则先插入新记录  
				String str2 = "INSERT INTO browseWorld(userID,hasBrowse) VALUES('"+userID+"',10);";
				pre = conn.prepareStatement(str2);
				pre.execute();
				
				//再返回新闻数据（json） 每次十条
				int i = 1;
				List<newsOfWorld> list = null ;
				while(i<=10){
					list.add(new newsOfWorld(i));
					i++;
				}
				...
				int i = 1;
				while(i<=10){
					list.add(new newsOfWorld(browser));
					i++;
